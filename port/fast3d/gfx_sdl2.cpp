...
static void gfx_sdl_init(const struct GfxWindowInitSettings *set) {
    ...
    {
        /* PORT_VSYNC=0 disables vsync (frame pacing then comes solely from
         * the port scheduler's 60Hz retrace clock). Default on. */
        const char *e = SDL_getenv("PORT_VSYNC");
        int interval = (e != nullptr && e[0] == '0') ? 0 : 1;
