...
extern "C" uint32_t gfx_dbg_tris_in, gfx_dbg_tris_out, gfx_dbg_vtx, gfx_dbg_mtx;
uint32_t gfx_dbg_vtxdump;
...
/* PORT_MAX_FLUSHES=<n>: debug bisect — drop all draw batches from the n-th
 * flush of each frame onward (counter reset in gfx_run). */
static int gfx_dbg_flush_cap = -2;
static int gfx_dbg_flush_count;

extern "C" uint32_t gfx_dbg_last_prim;
uint32_t gfx_dbg_last_prim;

static void gfx_flush(void) {
    if (buf_vbo_len > 0) {
        gfx_dbg_last_prim = (rdp.prim_color.r << 24) | (rdp.prim_color.g << 16) |
                            (rdp.prim_color.b << 8) | rdp.prim_color.a;
        if (gfx_dbg_flush_cap == -2) {
            const char* e = getenv("PORT_MAX_FLUSHES");
            gfx_dbg_flush_cap = e ? atoi(e) : -1;
        }
        if (gfx_dbg_flush_cap < 0 || gfx_dbg_flush_count < gfx_dbg_flush_cap) {
...
static inline void palette_to_rgba32(uint16_t palentry, uint8_t *rgba32_buf) {
    {
        /* PORT_PAL_SWAP=1: A/B debug — byte-swap palette entries */
        static int palswap = -1;
        if (palswap < 0) {
            const char *e = getenv("PORT_PAL_SWAP");
            palswap = e != NULL ? atoi(e) : 0;
        }
        if (palswap) palentry = (uint16_t)((palentry >> 8) | (palentry << 8));
    }
    if (rdp.palette_fmt == G_TT_IA16) {
        /* entries are PD_BE16'd at load: native value = I<<8 | A */
...
static void import_texture_ci4(int tile, const LoadedTexture& loaded_texture, bool gen_mipmaps) {
    ...
    if (getenv("PORT_CI_TRACE") != NULL) {
        fprintf(stderr, "port/ci4: addr=%p texnum=%u size=%u pal_idx=%u pal[0..3]=%04x %04x %04x %04x out0=%02x%02x%02x%02x\n",
                addr, *(const uint16_t *)(addr - 8), size_bytes, pal_idx,
                palette[0], palette[1], palette[2], palette[3],
                tex_upload_buffer[0], tex_upload_buffer[1], tex_upload_buffer[2], tex_upload_buffer[3]);
    }
...
static void import_texture_ci8(int tile, const LoadedTexture& loaded_texture, bool gen_mipmaps) {
    ...
    if (getenv("PORT_CI_TRACE") != NULL) {
        ...
        for (uint32_t i = 0; i < size_bytes; i++) {
            rs += tex_upload_buffer[4 * i]; gs += tex_upload_buffer[4 * i + 1]; bs += tex_upload_buffer[4 * i + 2];
        }
        fprintf(stderr, "port/ci8: addr=%p texnum=%u size=%u pal[2..3]=%04x %04x avg=%u,%u,%u\n",
                addr, *(const uint16_t *)(addr - 8), size_bytes,
                rdp.palette[2], rdp.palette[3],
...
static void import_texture(int i, int tile, bool importReplacement) {
    ...
    if (loaded_texture.line_size_bytes == 0 || loaded_texture.size_bytes == 0) {
        // Degenerate tile (e.g. the game's texture pool overflowed and the
        // load was dropped) — skip rather than divide by zero.
        static int warned;
        if (warned < 8) {
            fprintf(stderr, "port/warn: skipping zero-sized texture import (tile %d fmt %d siz %d)\n",
                    tile, fmt, siz);
            warned++;
        }
        return;
...
static void gfx_sp_matrix(uint8_t parameters, const int32_t* addr) {
    gfx_dbg_mtx++;
    if (addr == NULL) {
        // GE port: the game occasionally submits a NULL matrix (N64 DMA
        // from address 0 just reads RDRAM); ignore it
        return;
    }
    float matrix[4][4];
    ...
    {
        extern uint32_t gfx_dbg_vtxdump;
        if (gfx_dbg_vtxdump > 0) {
            fprintf(stderr, "port/mtx: addr=%p raw[%08x %08x %08x %08x %08x %08x %08x %08x]\n",
                    (const void *)addr,
                    (unsigned)addr[0], (unsigned)addr[1], (unsigned)addr[2], (unsigned)addr[3],
                    (unsigned)addr[8], (unsigned)addr[9], (unsigned)addr[10], (unsigned)addr[11]);
            fprintf(stderr, "port/mtx: %s%s%s [%.2f %.2f %.2f %.2f | %.2f %.2f %.2f %.2f | %.2f %.2f %.2f %.2f | %.2f %.2f %.2f %.2f]\n",
    ...
    if (parameters & G_MTX_PROJECTION) {
        ...
        if (rsp.modelview_matrix_stack_size == 0) {
            // GE room DLs can pop the stack empty and then LOAD; without
            // this the write lands 64 bytes BEFORE the stack (it corrupted
            // the color-combiner cache statics).
...
static void gfx_sp_vertex(size_t n_vertices, size_t dest_index, const Vtx* vertices) {
    gfx_dbg_vtx += n_vertices;
    SUPPORT_CHECK(n_vertices <= MAX_VERTICES);

    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        // GE port: GoldenEye uses the standard 16-byte SGI Vtx with inline
        // colors (unlit) or packed normals+alpha (lit), unlike PD's 12-byte
        // Rare vertex with a color-array index.
        const Vtx* vu = &vertices[i];
        const Vtx_t* v = &vu->v;
        ...
        {
            extern uint32_t gfx_dbg_vtxdump;
            if (gfx_dbg_vtxdump > 0 && i == 0) {
                gfx_dbg_vtxdump--;
                fprintf(stderr, "port/vtx: ob(%d,%d,%d) -> xyzw(%.1f, %.1f, %.1f, %.2f)\n",
        ...
        struct NormalColor vcnLocal;
        if (rsp.geometry_mode & G_LIGHTING) {
            vcnLocal.x = (int8_t)vu->n.n[0];
            vcnLocal.y = (int8_t)vu->n.n[1];
            vcnLocal.z = (int8_t)vu->n.n[2];
            vcnLocal.a = vu->n.a;
        } else {
            vcnLocal.r = v->cn[0];
            vcnLocal.g = v->cn[1];
        ...
        const struct NormalColor *vcn = &vcnLocal;
        ...
        if (rsp.geometry_mode & G_LIGHTING) {
            ...
            if (rsp.geometry_mode & G_TEXTURE_GEN) {
                ...
                {
                    /* PORT_TEXGEN_FLIP: A/B debug — bit1 flips U, bit2 flips V */
                    static int flip = -1;
                    if (flip < 0) {
                        const char *e = getenv("PORT_TEXGEN_FLIP");
                        flip = e != NULL ? atoi(e) : 0;
                    }
                    if (flip & 1) dotx = -dotx;
                    if (flip & 2) doty = -doty;
                }
...
static void gfx_sp_tri1(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx, bool is_rect) {
    ...
    if (gfx_dbg_tridump > 0 && !is_rect) {
        ...
        fprintf(stderr, "port/tri:   tile0 fmt=%u siz=%u wh=%ux%u tmem=%u addr=%p sz=%u first8=%02x%02x%02x%02x%02x%02x%02x%02x\n",
                rdp.texture_tile[0].fmt, rdp.texture_tile[0].siz,
                rdp.texture_tile[0].width, rdp.texture_tile[0].height,
                rdp.texture_tile[0].tmem,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].size_bytes,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr ? rdp.loaded_texture[rdp.texture_tile[0].tmem].addr[0] : 0,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr ? rdp.loaded_texture[rdp.texture_tile[0].tmem].addr[1] : 0,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr ? rdp.loaded_texture[rdp.texture_tile[0].tmem].addr[2] : 0,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr ? rdp.loaded_texture[rdp.texture_tile[0].tmem].addr[3] : 0,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr ? rdp.loaded_texture[rdp.texture_tile[0].tmem].addr[4] : 0,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr ? rdp.loaded_texture[rdp.texture_tile[0].tmem].addr[5] : 0,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr ? rdp.loaded_texture[rdp.texture_tile[0].tmem].addr[6] : 0,
                rdp.loaded_texture[rdp.texture_tile[0].tmem].addr ? rdp.loaded_texture[rdp.texture_tile[0].tmem].addr[7] : 0);
    }
    ...
    uint16_t zmode = rdp.other_mode_l & ZMODE_DEC;
    {
        static int no_z = -1;
        if (no_z < 0) {
            no_z = getenv("PORT_NO_Z") != NULL;
        }
        if (no_z) {
            depth_test = depth_compare = depth_update = false;
        }
    }
    ...
    {
        extern uint32_t gfx_dbg_ccdump;
        if (gfx_dbg_ccdump > 0) {
            gfx_dbg_ccdump--;
            fprintf(stderr, "port/cc-in: cc=%016llx opts=%016llx sid0=%016llx sid1=%08x n_in=%d tex=%d,%d map={",
                    (unsigned long long)rdp.combine_mode, (unsigned long long)cc_options,
                    (unsigned long long)comb->shader_id0, comb->shader_id1,
                    num_inputs, used_textures[0], used_textures[1]);
            for (int k = 0; k < 2; k++)
                for (int j = 0; j < 4; j++)
                    fprintf(stderr, "%d%s", comb->shader_input_mapping[k][j], (k == 1 && j == 3) ? "" : ",");
            fprintf(stderr, "} prim=%02x%02x%02x%02x vcol=%02x%02x%02x%02x 2cyc=%d\n",
                    rdp.prim_color.r, rdp.prim_color.g, rdp.prim_color.b, rdp.prim_color.a,
                    v1->color.r, v1->color.g, v1->color.b, v1->color.a, use_2cyc);
        }
    }
...
static void gfx_sp_moveword(uint8_t index, uint16_t offset, uintptr_t data) {
    switch (index) {
        ...
        case G_MW_FOG:
            static int no_fog = -1;
            if (no_fog < 0) {
                no_fog = getenv("PORT_NO_FOG") != NULL;
            }
            if (no_fog) {
                rsp.fog_mul = 0;
                rsp.fog_offset = 0;
...
static void gfx_dp_set_texture_image(uint32_t format, uint32_t size, uint32_t width, uint32_t tex_flags, const void* addr) {
    {
        extern uint32_t gfx_dbg_tridump;
        if (gfx_dbg_tridump > 0) {
            fprintf(stderr, "port/timg: addr=%p fmt=%u siz=%u width=%u\n", addr, format, size, width);
        }
    }
...
static void gfx_dp_load_tlut(uint8_t tile, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t lrt) {
    ...
    if (getenv("PORT_TLUT_TRACE") != NULL) {
        fprintf(stderr, "port/tlut: texnum=%u fmt=%04x img=%p uls=%u count=%u pal[0..3]=%04x %04x %04x %04x\n",
                *(const uint16_t *)((const uint8_t *)rdp.texture_to_load.addr - 8),
                rdp.palette_fmt, rdp.texture_to_load.addr, uls, count,
                PD_BE16(base[0]), PD_BE16(base[1]), PD_BE16(base[2]), PD_BE16(base[3]));
    }
...
static void gfx_dp_set_prim_color(uint8_t m, uint8_t l, uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
    {
        extern uint32_t gfx_dbg_ccdump;
        if (gfx_dbg_ccdump > 0) {
            fprintf(stderr, "port/prim: set %02x%02x%02x%02x m=%u l=%u\n", r, g, b, a, m, l);
        }
    }
...
extern uint32_t gfx_dbg_rectdump;
uint32_t gfx_dbg_rectdump;
...
static void gfx_dp_texture_rectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry, uint8_t tile, int16_t uls,
    ...
    // GE port: the game draws 1-scanline strips with lry == uly (the RDP
    // still rasterizes the span containing the subpixel); at scaled
    // resolutions that rounds to zero height. Pad degenerate rects to one
    // native pixel (4 units in 10.2).
    if (lry - uly < 4) {
        lry = uly + 4;
    }
    if (lrx - ulx < 4) {
    ...
    if (gfx_dbg_rectdump > 0) {
        gfx_dbg_rectdump--;
        fprintf(stderr, "port/rect: tex (%d,%d)-(%d,%d) tile=%u st=(%d,%d) tex=%p env=%02x%02x%02x%02x prim=%02x%02x%02x%02x cc=%08x-%08x om=%08x:%08x\n",
                ulx >> 2, uly >> 2, lrx >> 2, lry >> 2, tile, uls, ult,
                (void*)rdp.texture_tile[tile].line_size_bytes, /* proxy */
                rdp.env_color.r, rdp.env_color.g, rdp.env_color.b, rdp.env_color.a,
                rdp.prim_color.r, rdp.prim_color.g, rdp.prim_color.b, rdp.prim_color.a,
                (unsigned)(rdp.combine_mode >> 32), (unsigned)rdp.combine_mode,
                rdp.other_mode_h, rdp.other_mode_l);
...
static void gfx_dp_fill_rectangle(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry) {
    if (rdp.color_image_address == rdp.z_buf_address) {
        // The game is clearing its Z buffer the N64 way (fill rect drawn
        // into the depth image). PD's port replaced these with
        // G_CLEAR_DEPTH_EXT; GE's unmodified code path lands here, so do
        // the equivalent GL depth clear.
        gfx_flush();
...
static inline void *seg_addr(uintptr_t w1) {
    // GE port: the game passes raw N64 segment addresses (0x0S00xxxx) at
    // runtime (e.g. &dlBasicGeometry == 0x01000000 from rom_symbols.ld), so
    // treat any address below 0x10000000 with a registered segment as
    // segmented. Real pointers can't collide: ELF static data sits at
    // 0x08xxxxxx (segments 7..12 are never registered by GE) and the heap
    // is at 0xd0000000+. The PD-style LSB tag is honored too.
    if (w1 < 0x10000000 || (w1 & 1)) {
...
static void gfx_run_dl(Gfx* cmd) {
    ...
    for (;;) {
        ...
        switch (opcode) {
                // RSP commands:
            case G_NOOP: /* == GE G_SETTEX 0xC0; a real SETTEX carries payload */
                if ((cmd->words.w0 & 0xFFFFFF) != 0 || cmd->words.w1 != 0) {
                    static int settexWarn;
                    if (settexWarn < 8) {
                        settexWarn++;
                        sysLogPrintf(LOG_WARNING, "raw G_SETTEX reached the renderer (w0 %08x w1 %08x) - DL not rewritten",
                                     (unsigned)cmd->words.w0, (unsigned)cmd->words.w1);
                    }
                }
                break;
            case G_MTX: {
            ...
            case (uint8_t)G_SETOTHERMODE_H:
                ...
                break;
#ifdef G_COL /* PD-only: GE vertices carry inline colors */
            case G_COL:
            ...
            case G_SETCOMBINE:
            {
                extern uint32_t gfx_dbg_tridump;
                if (gfx_dbg_tridump > 0) {
                    fprintf(stderr, "port/cc: raw %08x %08x\n", (unsigned)cmd->words.w0, (unsigned)cmd->words.w1);
                }
            }
            ...
            default:
                {
                    // GE port: stale/garbage DLs are tolerated on N64 (the
                    // RSP just renders junk); warn and abandon this DL
                    static int warned;
                    if (warned < 20) {
                        warned++;
                        sysLogPrintf(LOG_WARNING, "unknown GBI opcode 0x%02x at %p (w0 %08x w1 %08x); abandoning DL",
...
static void gfx_sp_reset() {
    ...
    rsp.lights_changed = true;
    /* An N64 RSP task boots with clean DMEM state every frame; carrying
     * geometry mode (and lookat) across gfx_run calls leaks e.g. the menus'
     * G_LIGHTING into gameplay DLs that never set it — GE's first-person gun
     * then lights its prelit white vertex colors as garbage normals with
     * stale lights (blue/green tinted gun, "inverted" highlights). */
    rsp.geometry_mode = 0;
...
extern "C" void gfx_run(Gfx* commands) {
...

    {
        static int fc = -2;
        if (fc == -2) {
            const char* e = getenv("PORT_FINAL_PROBE");
            fc = e ? atoi(e) : -1;
        }
        if (fc > 0) {
            fc--;
            extern void gfx_dbg_final_probe(void);
            gfx_dbg_final_probe();
        }
    }
    gfxFramebuffer = 0;
