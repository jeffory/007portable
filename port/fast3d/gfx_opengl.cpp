...
static void gfx_opengl_upload_texture(const uint8_t* rgba32_buf, uint32_t width, uint32_t height, bool gen_mipmaps) {
    {
        /* PORT_UPLOAD_DUMP=<dir>: write every texture upload as raw RGBA32
         * (named upNNNN_WxH.raw) for offline inspection */
        static const char *dir = (const char *)-1;
        if (dir == (const char *)-1) dir = getenv("PORT_UPLOAD_DUMP");
        if (dir != NULL) {
            static int n;
            char path[256];
            snprintf(path, sizeof(path), "%s/up%04d_%ux%u.raw", dir, n++, width, height);
            FILE *f = fopen(path, "wb");
            if (f) { fwrite(rgba32_buf, 4, (size_t)width * height, f); fclose(f); }
        }
    }
...
static void gfx_opengl_draw_triangles(float buf_vbo[], size_t buf_vbo_len, size_t buf_vbo_num_tris) {
    ...
    /* PORT_GL_TRACE: per-draw GL state + framebuffer probe (debug) */
    {
        static int trace = -1;
        if (trace == -1) {
            const char* e = getenv("PORT_GL_TRACE");
            trace = e ? atoi(e) : 0;
        }
        if (trace > 0) {
            static int t0_set; static uint64_t t0;
            struct timespec ts;
            clock_gettime(CLOCK_MONOTONIC, &ts);
            uint64_t now = (uint64_t)ts.tv_sec * 1000000000ull + ts.tv_nsec;
            if (!t0_set) { t0 = now; t0_set = 1; }
            double secs = (now - t0) / 1e9;
            const char* after = getenv("PORT_GL_TRACE_AFTER");
            if (!after || secs >= atof(after)) {
                trace--;
                GLint vp[4], sc[4], fbo = 0, prog = 0, cfm = 0, ffc = 0;
                GLboolean dm, sct = glIsEnabled(GL_SCISSOR_TEST);
                GLboolean dt = glIsEnabled(GL_DEPTH_TEST), bl = glIsEnabled(GL_BLEND);
                GLboolean cl = glIsEnabled(GL_CULL_FACE);
                ...
                /* count non-black pixels over the whole viewport */
                static unsigned char* fbbuf;
                if (!fbbuf) fbbuf = (unsigned char*)malloc(1920u * 1080u * 4u);
                int nb = 0, w = vp[2] > 1920 ? 1920 : vp[2], h = vp[3] > 1080 ? 1080 : vp[3];
                unsigned maxpx = 0;
                glReadPixels(vp[0], vp[1], w, h, GL_RGBA, GL_UNSIGNED_BYTE, fbbuf);
                for (int k = 0; k < w * h; k++) {
                    unsigned v = fbbuf[k*4] | fbbuf[k*4+1] | fbbuf[k*4+2];
                    if (v > 8) { nb++; if (v > maxpx) maxpx = v; }
                }
                size_t stride = buf_vbo_len / (3 * buf_vbo_num_tris);
                fprintf(stderr,
                        "port/gl: draw %u tris stride=%u prog=%d depth=%d blend=%d prim=%08x nonblack=%d max=%02x v0=[",
                        (unsigned)buf_vbo_num_tris, (unsigned)stride, prog,
                        dt, bl, ({ extern uint32_t gfx_dbg_last_prim; gfx_dbg_last_prim; }), nb, maxpx);
                for (size_t k = 0; k < stride && k < 26; k++) {
                    fprintf(stderr, "%s%.3f", k ? "," : "", buf_vbo[k]);
                }
                fprintf(stderr, "] v1=(%.2f,%.2f,%.2f,%.2f) v2=(%.2f,%.2f,%.2f,%.2f)\n",
                        buf_vbo[stride], buf_vbo[stride+1], buf_vbo[stride+2], buf_vbo[stride+3],
                        buf_vbo[stride*2], buf_vbo[stride*2+1], buf_vbo[stride*2+2], buf_vbo[stride*2+3]);
            }
        }
    }
}

extern "C" void gfx_dbg_final_probe(void) {
    GLint vp[4];
    glGetIntegerv(GL_VIEWPORT, vp);
    static unsigned char* fbbuf;
    if (!fbbuf) fbbuf = (unsigned char*)malloc(1920u * 1080u * 4u);
    int w = vp[2] > 1920 ? 1920 : vp[2], h = vp[3] > 1080 ? 1080 : vp[3];
    glReadPixels(vp[0], vp[1], w, h, GL_RGBA, GL_UNSIGNED_BYTE, fbbuf);
    int nb = 0; unsigned maxpx = 0;
    for (int k = 0; k < w * h; k++) {
        unsigned v = fbbuf[k*4] | fbbuf[k*4+1] | fbbuf[k*4+2];
        if (v > 8) { nb++; if (v > maxpx) maxpx = v; }
    }
    {
        ...
        fprintf(stderr, "port/final: t=%ld.%03ld end-of-frame nonblack=%d max=%02x vp=%dx%d\n",
                (long)ts.tv_sec, ts.tv_nsec / 1000000, nb, maxpx, w, h);
    }
    if (getenv("PORT_RED_TEST") != NULL) {
        glDisable(GL_SCISSOR_TEST);
        glClearColor(1.0f, 0.0f, 0.0f, atof(getenv("PORT_RED_TEST")));
