...
void aEnvMixerImpl(uint8_t flags, ENVMIX_STATE state) {
    ...
    /* GE port: Rare's aspMain envelope is ADDITIVE-linear, not sm64's
     * multiplicative-exponential. env.c _getRate/_getVol define the
     * aSetVolume A_RATE pair (ratm<<16|ratl) as a SIGNED Q16.16 volume
     * INCREMENT applied per 8-sample group (vol += rate * samples/8),
     * clamped at the target; ratm=-0x8000 / 0x7fff mean "snap to target
     * now". The vendored sm64 code read it as a Q16.16 multiplier, so
     * steady-state voices (which pass the snap values) overflowed
     * `vol * (rate - 1.0)` into a garbage 8-lane volume staircase on
     * every A_INIT — audible as a constant ~2.7 kHz whine under music.
     * Scalar-only rewrite (the SIMD variants were dropped with it). */
    int16_t target[2];
    int32_t rate[2];
...

    {
        static int trace = -1;
        if (trace == -1) {
            const char *e = getenv("PORT_ENV_TRACE");
            trace = e ? (int)strtol(e, 0, 10) : 0;
        }
        if (trace > 0) {
            trace--;
            fprintf(stderr,
                    "port/env: fl=%02x nb=%u vol=%d,%d tgt=%d,%d rate=%.4f,%.4f dry=%d wet=%d\n",
                    flags, rspa.nbytes,
                    rspa.vol[0], rspa.vol[1], rspa.target[0], rspa.target[1],
                    (double)rspa.rate[0] / 65536.0, (double)rspa.rate[1] / 65536.0,
                    rspa.vol_dry, rspa.vol_wet);
        }
    }
    ...
    if (flags & A_INIT) {
        ...
        for (i = 0; i < 8; i++) {
            vols[0][i] = clamp32((int64_t)((int32_t)rspa.vol[0] << 16) + (int64_t)step[0] * (i + 1));
            vols[1][i] = clamp32((int64_t)((int32_t)rspa.vol[1] << 16) + (int64_t)step[1] * (i + 1));
        }
    } else {
    ...
    do {
        for (c = 0; c < 2; c++) {
            for (i = 0; i < 8; i++) {
                if (rate[c] >= 0) {
                    /* rising (or holding): don't overshoot the target */
                    if ((vols[c][i] >> 16) > target[c]) {
                        vols[c][i] = target[c] << 16;
                    }
                } else {
                    /* falling: don't undershoot the target */
                    if ((vols[c][i] >> 16) < target[c]) {
                        vols[c][i] = target[c] << 16;
                    }
                }
                ...
                /* each lane advances 8 samples per iteration */
