...
/*
 * Monitor-animation bytecode word (the monAnim* tables in propobj.c and
 * the tvcmd interpreter). The stream mixes opcodes, numeric operands and
 * POINTERS to sibling routines (MONJUMPTO/MONJUMPCHANCE), all in one
 * word-array. On N64/32-bit that word is u32; a 64-bit host needs a
 * pointer-wide word because C forbids truncating a pointer in a static
 * initializer. Numeric operands keep s32 semantics (negative scrolls),
 * so table macros sign-extend them via MONWORDVAL.
 */

/*
 * Model rwdata slots: RwDataIndex counts 4-byte words (modelCalculateRwData-
 * Indexes accumulates sizeof(rwdata)/4). On N64/PC32 `&datas[index]` happens
...
#pragma region AnimationStuff
    ...
    typedef struct ModelAnimation
    {
         ...
         * members become u32 slots holding truncated pointers (all game
         * memory sits below 4GB); the only deref sites are in model.c. */
